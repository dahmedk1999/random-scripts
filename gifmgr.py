...
def processImage(path):
    ...
    try:
        while True:
            ...
            new_frame.save('%s-%d.png' % (path, i), 'PNG')
            pathstoframes.append('%s-%d.png' % (path,i))
            i += 1
    ...
    for anims in png_dir: # [ [],[],[] ]
        ...
        for frames in anims: # [ [ , , , ] ]
            if frames.endswith('.png'):
                file_path = frames.replace("input","output") #take images in output folder
